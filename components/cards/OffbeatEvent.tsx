...
function OffbeatEvent({img,eventTitle,eventDate,eventLocation,eventDescription}:OffbeatEventProps) {
  return (
    <div className="focusNews h-auto w-[90dvw]  md:w-[580px] flex flex-col border-2 border-[rgba(34,34,34,0.1)] p-4 space-y-3 transition bg-white rounded-lg">
        <Image className="h-auto w-full " src={img} height={960} width={600} alt="news image" />
    ...
    <h3 className="text-buttonPurple font-bold text-lg">
        {eventTitle}
    </h3>
    <p className="whitespace-normal text-left break-all">
  {eventDescription.split(' ').slice(0, 20).join(' ')} {/* Display only the first 15 words */}
  {eventDescription.split(' ').length > 20 && '...'} {/* Add ellipsis if text is longer than 15 words */}
</p>
